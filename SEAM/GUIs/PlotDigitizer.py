...
class Figure_Digitizer:
    ...
    def populate_left_panel(self):
        ...
        ttk.Button(view_settings_frame, text="Crosshair color", command=self.change_crosshair_color).pack(side="left", padx=5, pady=2)
        
        magnifier_frame = ttk.Frame(view_settings_frame)
        magnifier_frame.pack(side="left", padx=5, pady=2, fill="x", expand=True)
        
        ttk.Label(magnifier_frame, text="Magnifier:").pack(side="left")
        self.magnifier_var = tk.StringVar(value="5")
        magnifier_spinbox = ttk.Spinbox(magnifier_frame, from_=1, to=20, textvariable=self.magnifier_var, width=5)
        magnifier_spinbox.pack(side="left", padx=5)
        magnifier_spinbox.bind("<<Increment>>", self.update_magnification)
        magnifier_spinbox.bind("<<Decrement>>", self.update_magnification)
        magnifier_spinbox.bind("<Return>", self.update_magnification)
        magnifier_spinbox.bind("<FocusOut>", self.update_magnification)
